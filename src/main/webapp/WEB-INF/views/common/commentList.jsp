				...
				<form:form modelAttribute="qnaCommentVO" method="post" action="./update" id="updateForm">
					<div class="comment_box">
						<ul class="cmt_list">
							...
							<c:set var="tempDepth" value="0"></c:set>
							<c:forEach items="${list}" var="qnaCommentVO">
								...
								<c:forEach items="${list}" var="vo">
									...
									<ul>
										...
										<li id="${qnaCommentVO.id}">${qnaCommentVO.writer}</li>
										...
										<p>
											...
											<c:choose>
												...
												<c:when test="${qnaCommentVO.id ne qnaCommentVO.ref}">
													<span>${qnadepth}</span><span class=form-control>
														${qnaCommentVO.contents}</span>
												</c:when>
												<c:otherwise>
										...
										<div class="col row">
											<button type="button" class="btn btn-danger upup"
												data-qna-down="${qnaCommentVO.id}">댓글</button>
	
	
											<c:if test="${memberVO.accountId eq qnaCommentVO.writer}">
	
												<button type="button" class="btn btn-primary del" id="commentListResult"
													data-qna-qna="${qnaCommentVO.id}"
													style="width : 58px; height : 38px; ">삭제</button>
	
												<button type="button" class="btn btn-primary edit" id="contentsConfirm"
													data-comment-num="${qnaCommentVO.id}">수정</button>
